...
推荐按下面流程触发 setup（自然语言可直接照这个走）：

1. 先列出可接入 subagent 清单（预设）：

```bash
python3 scripts/setup.py --list-catalog
```

2. 用户选择一个或多个 agent 后，按选择顺序执行 setup：
...
3. setup 会对每个选中的 agent 执行：
   - 检查 runner 是否可用
   - 缺失时自动尝试安装（可用 `--no-install-missing` 关闭）
   - 安装后连通探测（`initialize` + `session/new`）
   - 展示并配置权限相关选项（默认是 mode，必要时可扩展）
   - 成功后继续下一个 agent
...
- 可通过 `--agents` 选择本次接入的多个 subagent；setup 会顺序执行安装与权限配置。
